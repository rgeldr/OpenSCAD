...
internalboxBottomHeightZMm = 30; // .1
...
// The thickness of the box top and floor.
boxBottomThicknessMm = 2.0; // [1:0.1:20]
...
boxTopHeightZMm = internalBoxTopHeightZMm + boxBottomThicknessMm;
// The height of the box bottom
boxBottomHeightZMm = internalboxBottomHeightZMm + boxBottomThicknessMm;
...
// ***********************************************************************************************************
// **** Create the objects for testing the fit of the gasket on YOUR printer before printing the full box **** // ***********************************************************************************************************
...
module BoxTop(isStdHinge) {
    difference() {
        ...
        rotate([180,0,0]) translate([0,-boxLengthYMm,-openingTolerance]) 
            difference() {
                translate([0,0,-(boxTopHeightZMm-boxBottomThicknessMm-.01)]) cube([boxWidthXMm, boxLengthYMm, boxTopHeightZMm-boxBottomThicknessMm-.02]);
...
module BoxBottom(isStdHinge) {
    difference() {
        ...
        // TODO: Cut off anything in the inside of the case
        difference() {
            translate([0,0,-(boxBottomHeightZMm-boxBottomThicknessMm-.01)]) cube([boxWidthXMm, boxLengthYMm, boxBottomHeightZMm-boxBottomThicknessMm-.02]);
...
module CreateBoxInserts(generateBottomInsert = true, generateTopInsert = true) {
    ...
    if(generateBottomInsert) {
        // TPU Top box insert
        translate([boxWidthXMm + 20, 0, boxBottomHeightZMm-boxBottomThicknessMm-insertTolerance]) 
                    resize([boxWidthXMm-(boxWallWidthMm*2)-(insertTolerance*2),boxLengthYMm-(boxWallWidthMm*2)-(insertTolerance*2),boxBottomHeightZMm-boxBottomThicknessMm-insertTolerance]) 
                    difference() { 
                        translate([0,0,-(boxBottomHeightZMm-boxBottomThicknessMm-.01)]) cube([boxWidthXMm, boxLengthYMm, boxBottomHeightZMm-boxBottomThicknessMm-.02]);
...
module BoxLengthXSeparators(separatorWidth, height, isSkipFromEnd) {
    ...
    if(countainerWidthXSections > 1) {
        for (y =[1+skipFromBegining:(countainerWidthXSections-1-skipFromEnd)]) {
            translate([(((boxWidthXMm-(2*boxWallWidthMm))/countainerWidthXSections)*y)+(boxWallWidthMm-(separatorWidth/2)),boxWallWidthMm,-(height-boxBottomThicknessMm)]) 
...
module BoxWidthYSeparators(separatorWidth, height, isSkipFromEnd) {
    ...
    if(boxLengthYSections > 1) {
        for (y =[1+skipFromBegining:(boxLengthYSections-1-skipFromEnd)]) {
            translate([boxWallWidthMm,separatorWidth+(((boxLengthYMm-(2*boxWallWidthMm))/boxLengthYSections)*y)+(boxWallWidthMm-(separatorWidth/2)),-(height-boxBottomThicknessMm)])
                rotate([0,0,-90])
...
//translate([boxWidthXMm/2, boxWallWidthMm, -(boxBottomHeightZMm-boxWallWidthMm)])
    difference() {
        cube([separatorWidth,width-(2*boxWallWidthMm), height-boxBottomThicknessMm]);
        translate([-0.1,(boxChamferRadiusMm-boxWallWidthMm),height-boxBottomThicknessMm]) rotate([90,0,90]) linear_extrude(separatorWidth+0.2) Wall2D(boxWallWidthMm, boxChamferRadiusMm, height, false, false);
        translate([separatorWidth+0.1,width-(2*boxWallWidthMm),0]) rotate([0,0,180]) translate([0,(boxChamferRadiusMm-boxWallWidthMm),height-boxBottomThicknessMm]) rotate([90,0,90]) linear_extrude(separatorWidth+0.2) Wall2D(boxWallWidthMm, boxChamferRadiusMm, height, false, false);
    }
}
...
module BoxShellBase(height, additionalShellThickness = 0) {
    // Bottom
    translate([boxChamferRadiusMm,boxChamferRadiusMm,-height]) cube([boxWidthXMm-(2*boxChamferRadiusMm), boxLengthYMm-(2*boxChamferRadiusMm), boxBottomThicknessMm]);
    // Corners
        ...
        for(cnt = [0:additionCount]) {
            ...
            translate([-boxChamferRadiusMm-(additionXAdj),0])
                union() {
                    ...
                    translate([boxChamferRadiusMm-floorAddition, -height]) square([floorAddition, boxBottomThicknessMm]);
